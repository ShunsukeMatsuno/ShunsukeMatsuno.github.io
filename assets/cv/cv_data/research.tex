\newcommand{\co}{\textsuperscript{†}}

%-------------------------------------------------------------------------------
% SECTION TITLE
%-------------------------------------------------------------------------------
\cvsection{Research}


%-------------------------------------------------------------------------------
% Interests
%-------------------------------------------------------------------------------
\cvsubsection{Interests}

\begin{cventries}

\cventryLONG
	{Financial reporting and disclosure; information in financial markets; organizational economics with applications to managerial accounting; auditing}  

\end{cventries}

\vspace{.3cm}

%-------------------------------------------------------------------------------
% JMP
%-------------------------------------------------------------------------------
\cvsubsection{Job Market Paper}

\paperentry
	{The Gatekeeping Expert's Dilemma}   % title
	{Shunsuke Matsuno}   % author(s)
	{}   % year
	{\href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5583773}{[link]} \href{https://shunsukematsuno.github.io/papers/supplementary-material_Gatekeeping-Experts-Dilemma.pdf}{[supplementary material]}}  % paper link
	{}   % link journal
	{}   % Journal
	{
	\begin{cvitems}  % Presentations
		\item {Presented at: Rice, Stanford, Chicago Booth, Columbia, EAA Talent Workshop, Hawaii Accounting Research Conference, MFA Annual Meeting, SITE, CMU Mini Conference} 
		% \hfill\makebox[0pt][r]{\scriptsize(\co  Presented by coauthor)}}			
		\item {Media mention: CLS Blue Sky Blog \href{https://clsbluesky.law.columbia.edu/2025/11/12/how-strategic-vagueness-helps-gatekeepers-offer-guidance-without-being-gamed/}{[link]}}
	\end{cvitems}
	}
	{% Abstract
	This paper studies how experts with veto power---gatekeeping experts---influence agents through communication. Their expertise informs agents' decisions, while veto power provides discipline. Gatekeepers face a dilemma: transparent communication can invite gaming, while opacity wastes expertise. How can gatekeeping experts guide behavior without being gamed? Many economic settings feature this tradeoff, including bank stress tests, environmental regulations, and financial auditing. Using financial auditing as the primary setting, I show that strategic vagueness resolves this dilemma: by revealing just enough to prevent the manager from inflating the report, the auditor guides the manager while minimizing opportunities for manipulation. This theoretical lens provides a novel rationale for why auditors predominantly accept clients' financial reports. Comparative statics reveal that greater gatekeeper independence or expertise sometimes dampens communication. This paper offers insights into why gatekeepers who lack direct control can still be effective.
	}

\vspace{.5cm}
%-------------------------------------------------------------------------------
% Working Papers
%-------------------------------------------------------------------------------
% \cvsubsection{Working Papers}{(\co Presented by coauthor)}
\cvsubsection{Working Papers}

\paperentry
	{Mandatory vs. Voluntary Disclosure in the Dynamic Market for Lemons}   % title
	{Cyrus Aghamolla and Shunsuke Matsuno}   % author(s)
	{}   % year
	{\href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4680389}{[link]}}  % paper link
	{}   % link journal
	{R\&R at Review of Financial Studies}   % Journal
	{
	\begin{cvitems}  % Presentations
		\item {Presented at: Hawaii Accounting Research Conference, Columbia University, AES Winter Retreat, AES Annual Conference, Burton Accounting Conference} 
		% \hfill\makebox[0pt][r]{\scriptsize(\co  Presented by coauthor)}}			
		\item {Media mention: Duke FinReg Blog \href{https://sites.duke.edu/thefinregblog/2024/10/29/mandatory-vs-voluntary-disclosure-in-the-dynamic-market-for-lemons/}{[link]}}
	\end{cvitems}
	}
	{}

\paperentry
	{How Does Corporate Culture Create Value? Conformity and the Coordination of Dispersed Information}
	{Shunsuke Matsuno}
	{}   % year
	{\href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5291646}{[link]}}
	{}
	{R\&R at Management Science}
	{
	\begin{cvitems} % Presentations
		\item {Presented at: Columbia Theory Conference, Hawaii Accounting Research Conference, Kobe University Kanematsu Seminar, Zurich Accounting Research Workshop, AES Annual Conference}
	\end{cvitems}
	}
	{}

\paperentry
	{Strategic Patenting Under Financial Disclosure Mandates}
	{Li Azinovic-Yang, Yue Chen, Shunsuke Matsuno, Sunho Yoo}
	{}   % year
	{\href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=5767542}{[link]}}
	{}
	{}
	{
	\begin{cvitems} % Presentations
		\item {Presented at: Zurich Accounting Research Workshop, CUHK, EAA, Columbia University, Duke University, DISS Workshop, Utah Winter Accounting Conference}
	\end{cvitems}
	}
	{}

	\paperentry
	{PCAOB Monitoring and Auditor Effort: Evidence from Dynamic Model Estimation}
	{Lisa Yao Liu and Shunsuke Matsuno}
	{}   % year
	{\href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=6760918}{[link]}}
	{}
	{}
	{
	\begin{cvitems} % Presentations
		\item {Presented at: Columbia University, TAR/PCAOB Registered Reports Conference}
	\end{cvitems}
	}
	{}

\paperentry
	{Disclosure, Signaling, and First-Mover (Dis)advantage}   % title
	{Shunsuke Matsuno}   % author(s)
	{}   % year
	{\href{https://papers.ssrn.com/sol3/papers.cfm?abstract_id=4985567}{[link]}}  % paper link
	{}   % link journal
	{}   % Journal
	{
	\begin{cvitems}  % Presentations
		\item {Presented at: Hawaii Accounting Research Conference, Columbia University, Duke University Accounting Theory Summer School, Junior Accounting Theory Conference}
	\end{cvitems}
	}
	{}

% \vspace{.3cm}

%------------------------------------------------------------------------------
% SUBSECTION TITLE
%-------------------------------------------------------------------------------
% \cvsubsection{Work in Progress}

% \paperentry
% 	{PCAOB Inspections, Dynamic Enforcement, and Deterrent Effect: Evidence from Structural Estimation}
% 	{Lisa Liu \& Shunsuke Matsuno}
% 	{}
% 	{}
% 	{}
% 	{}
% 	{
% 	\begin{cvitems} % Description(s) of tasks/responsibilities
% 		\item {Presented at: 2024 PCAOB/TAR Registered Reports Conference\co}
% 		\item {Revising for resubmission at The Accounting Review (registered report)}
% 	\end{cvitems}
% 	}
% 	{}

% %%%%% If you want to use bibtex
% \cvsubsection{Published}
% \begin{refsection}
% 	\nocite{Selimi2015Cloud}
% 	\nocite{Khan2014Incentive}	
	
% 	\printbibliography[
% 	heading=none
% 	]
% \end{refsection}
